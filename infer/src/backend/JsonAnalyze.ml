...
module NameParsersToAnalyzer(NP: NameParsers) = struct
  ...
  let parse_ikind (json : Yojson.Safe.t) =
    let ikind_map =
  ...
  let parse_fkind (json : Yojson.Safe.t) =
  ...
  let parse_ptr_kind (json : Yojson.Safe.t) =
  ...
  let parse_if_kind (json : Yojson.Safe.t) =
  ...
  let parse_csu (json : Yojson.Safe.t) =
  ...
  let parse_binop (json : Yojson.Safe.t) = (*TODO: need to check the usage of "None" *)
  ...
  let parse_long (json: Yojson.Safe.t) =
  ...
  let parse_ident (json : Yojson.Safe.t) =
    let k = (to_string (member "kind" json)) in
    let res = 
      if String.equal k "Normal" then
        Ident.create_normal (Ident.string_to_name (to_string (member "name" json)))  (to_int (member "stamp" json))
      else if String.equal k "Primed" then
        Ident.create Ident.kprimed (to_int (member "stamp" json))
      else if String.equal k "Footprint" then
        Ident.create_footprint (Ident.string_to_name (to_string (member "name" json)))  (to_int (member "stamp" json))
  ...
  let parse_fieldident (json : Yojson.Safe.t) =
    NP.fieldname_of_string (to_string (member "field_name" json))
    
  let parse_source_file (json : Yojson.Safe.t) =
    let p = to_string (member "path" json) in
    SourceFile.create ~warn_on_error:false p

  let parse_location (json : Yojson.Safe.t) =
    {
  ...
  let rec parse_pvar (json : Yojson.Safe.t) =
    ...
    if String.equal pvkind "LocalVariable" then
      let pname = NP.parse_procname (member "proc_name" json) in
  ...
  and parse_constant (json : Yojson.Safe.t) =
    ...
    if String.equal const_kind "Int" then
      ...
      let pname = NP.parse_procname const_value in
      Const.Cfun pname
      ...
      match const_value with
        | `String _ -> Const.Cfloat Float.nan
        | `Float _ -> Const.Cfloat (to_float const_value) 
        | _ -> raise (Failure "Unmatched type when parsing constant")
  ...
  and parse_exp (json : Yojson.Safe.t) =
  ...
  and parse_struct_field (json : Yojson.Safe.t) =
    let fi = parse_fieldident json in
    let t = parse_sil_type_name  (member "type" json) in
    let annot = parse_item_annotation (member "annotation" json) in
    (fi, t, annot)

  and parse_sil_type_name (json : Yojson.Safe.t): Typ.t =
  ...
  and parse_param (json: Yojson.Safe.t): Annot.parameter =
    let parse_enum (json: Yojson.Safe.t): Annot.value = 
      let class_typ =(parse_sil_type_name (member "class_typ" json)) in
      let value = ((member "value" json) |> to_string) in 
      Annot.Enum {class_typ=class_typ;value=value}
    in
    let rec parse_value (json: Yojson.Safe.t): Annot.value =
      let value = (member "value" json) in
      match to_string (member "type" json) with 
        | "Str" -> Annot.Str (to_string value)
        | "Bool" -> Annot.Bool (to_bool value)
        | "Enum" -> parse_enum value
        | "Array" -> Array (parse_list parse_value value)
        | "Class" -> Class (parse_sil_type_name value)
        | "Annot" -> Annot (parse_annotation value)
        | _ -> raise (Failure "Unknown value in param")
    in
      {name=Some (to_string (member "name" json)); value=parse_value (member "value" json)}

  and parse_annotation (json : Yojson.Safe.t) =
    let class_name = to_string (member "class_name" json) in
    let p = member "params" json in
    let parameters = parse_list parse_param p in
    { Annot.class_name = class_name ; Annot.parameters = parameters }

  and parse_item_annotation (json : Yojson.Safe.t): Annot.Item.t =
  parse_list 
  (fun j -> 
  ...
  and parse_struct (json : Yojson.Safe.t) = 
    ...
    let methods = parse_list NP.parse_procname  (member "methods" json) in
    let annots = parse_item_annotation json in
    (fields, statics, supers, methods, annots)
        
  let parse_method_annotation (json : Yojson.Safe.t): Annot.Method.t =
  ...
  let parse_proc_attributes_var (json : Yojson.Safe.t) =
    let n = to_string (member "name" json) in
    let t = parse_sil_type_name  (member "type" json) in
    (Mangled.from_string n, t)

  
  let parse_captured_var (json: Yojson.Safe.t): CapturedVar.t = 
    let (m,t) = parse_proc_attributes_var json in
    let cap_mode = match to_string (member "capture_mode" json) with 
      | "ByReference" -> Pvar.ByReference
      | "ByValue" -> Pvar.ByValue
      | _ -> raise (Failure "Unknown capture mode parsing captured var")
  in {name=m;typ=t;capture_mode=cap_mode}

  let parse_proc_attributes_locals (json : Yojson.Safe.t) : ProcAttributes.var_data =
    ...
    let idu = to_bool (member "is_declared_unused" json) in
    { name= n
    ; typ= t
    ; modify_in_block= mib
    ; is_constexpr= ice
    ; is_declared_unused= idu}

  let parse_proc_attributes (json : Yojson.Safe.t) =
    let access = 
    ...
    let proc_name = NP.parse_procname (member "proc_name" json) in
    { 
  ...
  let parse_call_flags (json : Yojson.Safe.t) =
    {
  ...
  let parse_pdesc (cfg : Cfg.t) (pd_id_to_pd : Procdesc.t IntTbl.t) (start_nd_tbl : int IntTbl.t) (exit_nd_tbl : int IntTbl.t) (json : Yojson.Safe.t) =
  ...
  let parse_stmt_nodekind (json: Yojson.Safe.t): Procdesc.Node.stmt_nodekind =
    let parse_destruction_kind (json: Yojson.Safe.t): Procdesc.Node.destruction_kind = 
      match to_string json with 
        | "DestrBreakStmt" -> Procdesc.Node.DestrBreakStmt
        | "DestrContinueStmr" -> Procdesc.Node.DestrContinueStmt
        | "DestrFields" -> Procdesc.Node.DestrFields
        | "DestrReturnStmt" -> Procdesc.Node.DestrReturnStmt
        | "DestrScope" -> Procdesc.Node.DestrScope
        | "DestrTemporariesCleanup" -> Procdesc.Node.DestrTemporariesCleanup
        | "DestrVirtualBase" -> Procdesc.Node.DestrVirtualBase
        | _ -> raise (Failure "Unknown destruction kind")  
    in
    ...
    | "Destruction" ->
      Procdesc.Node.Destruction (parse_destruction_kind json)
    | "ExceptionHandler" ->
  ...
  let parse_prune_nodekind (json: Yojson.Safe.t): Procdesc.Node.prune_node_kind =
    match to_string json with 
    | "ExceptionHandler" ->
  ...
  let parse_cfg (json : Yojson.Safe.t) =
  ...
  let parse_tenv_type (json : Yojson.Safe.t) (tenv) =
    let tn = parse_typename (member "type_name" json) in
  ...
  let parse_tenv (json : Yojson.Safe.t) =
  ...
  let analyze_json cfg_json tenv_json =
    ...
    else DBWriter.delete_all_specs () ;
    ...
    let source_file = SourceFile.create ~warn_on_error:false "./Program.json" in
    ...
    Language.curr_language := NP.lang;
